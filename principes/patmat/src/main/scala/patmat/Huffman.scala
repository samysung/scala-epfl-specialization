...
object Huffman {
   ...
   */
    def times(chars: List[Char]): List[(Char, Int)] ={
      if(chars.isEmpty) List[(Char, Int)]()
        ...
        times(chars.distinct.tail)++List((chars.distinct.head,chars.count(c=>c==chars.distinct.head)))
      }
    }
  /**
   ...
   */
    /*def decode(tree: CodeTree, bits: List[Bit]): List[Char] ={
      def decodeHelper(toDecode:CodeTree,bits: List[Bit]):List[Char]=toDecode match{
        case Leaf(c,_)=>if(bits.isEmpty) List(c) else{c::decodeHelper(toDecode,bits)}
        case Fork(left,right,_,_)=> if(bits.head==0) decodeHelper(left,bits.tail) else{decodeHelper(right,bits.tail)}
    }
  ...
  def encode(tree: CodeTree)(text: List[Char]): List[Bit]= {
      def toEncode(remainTree:CodeTree,remainText:List[Char]):List[Bit]= {
        if (remainText.isEmpty) List[Bit]()
        else {
          remainTree match {
            case Leaf(_, _) => if (!text.tail.isEmpty) {
              println(text.head.toString); toEncode(tree, remainText.tail)
            } else List[Bit]()
            case Fork(left, right, chars, _) => if(this.chars(left).contains(remainText.head)) 0::toEncode(left, remainText) else 1::toEncode(right,remainText)
          }
        }
      }
      toEncode(tree,text)
   ...
   */
    def codeBits(table: CodeTable)(char: Char): List[Bit] = table.head._1==char match{
      case true=>table.head._2
   ...
   */
    def mergeCodeTables(a: CodeTable, b: CodeTable): CodeTable =
      {
        def foreach(ct:CodeTable,bit:Bit):CodeTable=ct.length match{
          case 0=>List()
          case 1=>List((ct.head._1,bit::ct.head._2))
          case _=>List((ct.head._1,bit::ct.head._2)):::foreach(ct.tail,bit)

        }
        foreach(a,0):::foreach(b,1)
      }
  /*
  ...
  */
   ...
   */
    def quickEncode(tree: CodeTree)(text: List[Char]): List[Bit] = {
      def foreach(ct:CodeTable,text: List[Char]):List[Bit]=text.length match{
        case 0=>List()
        case 1=>codeBits(ct)(text.head)
        case _=>codeBits(ct)(text.head):::foreach(ct,text.tail)
      }
      val ct=convert(tree);foreach(ct,text)
    }
  }
