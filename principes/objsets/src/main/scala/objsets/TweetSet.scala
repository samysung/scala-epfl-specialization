...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
  ...
  def filterAcc(p: Tweet => Boolean, acc: TweetSet): TweetSet = {
    ...
    else this.left.filterAcc(p,right.filterAcc(p,acc))
    }
  
  override def union(that:TweetSet):TweetSet={
    if(that.contains(this.elem)) this.left.union(this.right.union(that))
    else this.left.union(this.right.union(that.incl(elem)))
  }

  def descendingByRetweet: TweetList = {
    val mostR:Tweet=mostRetweeted
    new Cons(mostR, remove(mostR).descendingByRetweet)
